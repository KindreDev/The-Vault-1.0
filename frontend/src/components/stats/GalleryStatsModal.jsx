      ...
      {(isLoading || !d) ? <LoadingBody /> : (
        <>
          ...
          <div className="relative overflow-hidden rounded-[16px] mb-6"
               style={{ border: '0.5px solid color-mix(in srgb, var(--c-accent) 35%, transparent)',
                        boxShadow: '0 0 50px 8px color-mix(in srgb, var(--c-accent) 13%, transparent)' }}>
            {d.cover_thumb && (
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    {d.creators.map(c => (
                      <button key={c.id} onClick={() => go(`/creators/${c.id}`)}
                              ...
                              style={{ fontSize: 16, color: 'var(--c-accent-text)',
          ...
          <div className="grid gap-4" style={{ gridTemplateColumns: 'repeat(2, minmax(0,1fr))' }}>
            ...
            {/* ── Contents ─────────────────────────────────────────────── */}
            <Panel icon={Images} title="Contents" subtitle="What's in here" accent="var(--c-accent)">
            ...
            {/* ── Intensity ────────────────────────────────────────────── */}
            <Panel icon={TrendingUp} title="Intensity" subtitle="How hard it's been used" accent="var(--c-pink-text)">
              <div className="grid gap-3" style={{ gridTemplateColumns: 'repeat(3, minmax(0,1fr))' }}>
                <Stat label="Os / hour" value={d.os_per_hour ?? 0} accent="var(--c-pink-text)" />
                <Stat label="Os / view" value={d.os_per_view ?? 0} accent="var(--c-pink-text)" />
            ...
            {/* ── Standouts ────────────────────────────────────────────── */}
            <Panel icon={Star} title="Standouts" subtitle="The ones that earned it" accent="var(--c-amber-text)">
              <div className="flex flex-col gap-2">
                {d.most_gooned && (
                  <Standout label="Most-gooned" accent="var(--c-pink-text)"
                ...
                {d.most_viewed && (
                  <Standout label="Most-viewed" accent="var(--c-accent)"
                ...
                {d.longest_watched && (
                  <Standout label="Longest watched" accent="var(--c-accent-text)"
            ...
            {/* ── Curation & timeline ──────────────────────────────────── */}
            <Panel icon={Calendar} title="Curation & timeline" subtitle="How well kept, and since when" accent="var(--c-green)">
